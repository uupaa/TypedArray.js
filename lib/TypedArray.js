...
var TypedArray = {
    ...
    // debug
    "dump":         TypedArray_dump,            // TypedArray.dump(source:TypedArray|Array, options:Object = null):void
...
function TypedArray_dump(source,    // @arg TypedArray|Array - [0x00, 0x41, 0x53, 0x43, 0x49, 0x49, 0xff, ...]
                         options) { // @arg Object = null - { begin, end, radix, markup }
                                    // @options.begin  UINT32 = 0 - begin address
                                    // @options.end    UINT32 = source.length - end address
                                    // @options.radix  UINT8  = 16 - radix. 2 or 10 or 16
                                    // @options.markup Function = null - markup(index, num, source):StyleString
//{@dev
    if (!global["BENCHMARK"]) {
        ...
        $valid($type(options, "Object|omit"),      TypedArray_dump, "options");
        if (options) {
            $valid($type(options.begin,  "UINT32|omit"),   TypedArray_dump, "options.begin");
            $valid($type(options.end,    "UINT32|omit"),   TypedArray_dump, "options.end");
            $valid($type(options.radix,  "UINT8|omit"),    TypedArray_dump, "options.radix");
            $valid($type(options.markup, "Function|omit"), TypedArray_dump, "options.markup");
            if (options.radix) {
                $valid($some(options.radix + "", "2|10|16"),  TypedArray_dump, "options.radix");
            }
        }
    }
//}@dev
    ...
    var radix    = options["radix"]  || 16;
    var markup   = options["markup"] || null;
    ...
    result.push(DUMP_HEADER[radix][unitSize], _getHexAddress(begin), " ");
    ...
    for (var i = begin, iz = Math.min(end, source.length), x = 0; i < iz; ++i, x += unitSize) {
        ...
        if (ENABLE_CONSOLE_STYLE && markup) {
            var r = markup(i, v, source);
            if (r) {
                marked = true;
                result.push("%c");
                style.push(r);
        ...
        if (radix === 10 || radix === 2) {
            result.push( "(" + (padding[radix] + v.toString(radix)).slice( -(digit[radix][unitSize]) ) + ")" );
        }
        if (marked) {
            result.push("%c");
    ...
    if (unitSize === 1 && radix === 16 && String.prototype.repeat) { // dump to remaind ascii code
