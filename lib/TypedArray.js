...
var TypedArray = {
    ...
    // debug
    "dump":         TypedArray_dump,            // TypedArray.dump(source:TypedArray|Array, begin:UINT32 = 0, end:UINT32 = source.length, radix:UINT8 = 16, markup:Object|Array|Function = {}):void
    "hexDump":      TypedArray_hexDump,         // [DEPRECATED]
                                                // TypedArray.hexDump(source:TypedArray|Array, begin:UINT32 = 0, end:UINT32 = source.length):void
...
function TypedArray_dump(source,   // @arg TypedArray|Array - [0x00, 0x41, 0x53, 0x43, 0x49, 0x49, 0xff, ...]
                         begin,    // @arg UINT32 = 0 - begin address
                         end,      // @arg UINT32 = source.length - end address
                         radix,    // @arg UINT8 = 16 - radix. 16 or 10 or 2
                         markup) { // @arg Object|Array|Function = null - { num: color, ... } or [num, ...] or markup(num):StyleString
//{@dev
    if (!global["BENCHMARK"]) {
        ...
        $valid($type(markup, "Object|Array|Function|omit"), TypedArray_dump, "markup");
        if (radix) {
            $valid($some(radix + "", "2|10|16"),  TypedArray_dump, "radix");
        }
    }
//}@dev
    ...
    var markupfn = markup == null             ? null
                 : Array.isArray(markup)      ? _markupByArray
                 : markup instanceof Function ? _markupByFunction
                 : _markupByObject;
    ...
    var i = begin, iz = Math.min(end, source.length);
    var ascii = [];

    result.push(DUMP_HEADER[radix][SIZE], _getHexAddress(begin), " ");

    for (var x = 0; i < iz; ++i, x += SIZE) {
        if (i !== begin) {
            if ( x % maxDigit[radix] === 0 ) {
                // make ADDRESS
                if (SIZE === 1) { // add ascii dump
        ...
        if (ENABLE_CONSOLE_STYLE && markupfn) {
            var r = markupfn(i, v, markup);
            if (r) {
                marked = true;
                result.push("%c");
                color.push(r);
        ...
        if (SIZE === 1) { // make ascii code
            if (v >= 32 && v <= 126) {
                ascii.push(String.fromCharCode(v));
            } else {
              //ascii.push("\u00b7"); // center dot
                ascii.push("\uff65"); // center dot (half size dot)
            }
        }
    }
    if (SIZE === 1 && radix === 16 && String.prototype.repeat) { // dump to remaind ascii code
        var padLength = (16 - ascii.length) * 3 + 1;
        console.log.apply(console, [result.join("") + " ".repeat(padLength) + ascii.join("")].concat(color));
        return;
    }
    ...
    function _markupByArray(index, num, markup) {
        return markup.indexOf(num) >= 0 ? "font-weight:bold;color:red" : "";
    }
    function _markupByFunction(index, num, markup) {
        var result = markup(index, num, markup);
    ...
    function _markupByObject(index, num, markup) {
        if (num in markup) {
            return "font-weight:bold;color:" + markup[num];
        }
        var match = false;
        if ("min" in markup && "max" in markup) {
            match = num >= markup["min"] && num <= markup["max"];
        } else if ("min" in markup) {
            match = num >= markup["min"];
        } else if ("max" in markup) {
            match = num <= markup["max"];
        }
        return match ? "font-weight:bold;color:red" : "";
    }
}
