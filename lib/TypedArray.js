...
function TypedArray_dump(source,    // @arg TypedArray|Array - [0x00, 0x41, 0x53, 0x43, 0x49, 0x49, 0xff, ...]
                         options) { // @arg Object = null - { begin, end, radix, style }
...
//{@dev
    if (!global["BENCHMARK"]) {
        ...
        $valid($keys(options, "begin|end|radix|style"), TypedArray_dump, "options");
        if (options) {
            ...
            $valid($type(options.style, "Object|omit"), TypedArray_dump, "options.style");
            if (options.radix) {
...
//}@dev
    ...
    if (ENABLE_CONSOLE_STYLE && style) {
        for (var tag in style) {
            if ("values" in style[tag]) {
                ...
                //  }
                _seriesOfElements(css, source, style[tag]["values"],
                                               style[tag]["css"]);
            }
            if ("begin" in style[tag] || "end" in style[tag]) {
                //  style: {
    ...
    function _seriesOfElements(result, source, pattern, cssText) {
        for (var i = 0, iz = source.length; i < iz; ++i) {
            if (source[i] === pattern[0]) {
                // --- find the series of elements ---
                for (var x = 0, xz = pattern.length; x < xz; ++x) {
